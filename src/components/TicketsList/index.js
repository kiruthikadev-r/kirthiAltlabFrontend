...
function TicketsList() {
  ...
  const fetchTickets = async () => {
    if (user && user.uniqueId) {
      ...
      try {
        const response = await axios.get('https://kirthialtlabbackend-1.onrender.com/api/tickets/agent/tickets', {
          headers: {
  ...
  const updateTicketStatus = async (ticketId, newStatus) => {
    try {
      await axios.put(`https://kirthialtlabbackend-1.onrender.com/api/tickets/tickets/${ticketId}/status`, 
        { status: newStatus }, 
        {
          headers: {
            uniqueId: user.uniqueId,
  ...
  function Modal({ isOpen, onClose, ticket }) {
    ...
    const handleAddNote = async () => {
      if (noteContent.trim()) {
        try {
          const response = await axios.post(`https://kirthialtlabbackend-1.onrender.com/api/tickets/tickets/${ticket.ticketId}/notes`, 
            { content: noteContent }, 
            {
              headers: {
                uniqueId: user.uniqueId,
